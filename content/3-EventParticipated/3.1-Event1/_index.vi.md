...
#### Bài phát biểu của Banh Cam Vinh
...
- Kết nối **LLM** với thế giới thật qua **tools** (APIs, databases, external services) để biến **LLM** thành agent có khả năng hành động.
- Định nghĩa **AI Agent**: **LLM** làm "bộ não" cộng với **lập kế hoạch**, **gọi tools**, đọc kết quả và thực thi lặp đi lặp lại.
- **Strands Agents** cung cấp workflow chuẩn, bộ công cụ và chuẩn provider open-standard, giúp tăng tốc độ xây dựng **agentic loops**.

### Bài học rút ra

- Để **LLM** thực sự "làm việc", cần thiết kế **agent** chứ không chỉ tối ưu prompt.
- **Tool calling** và **agentic loop** là lõi: lập kế hoạch → chọn tool → gọi tool → đọc kết quả → lặp.
...
- Nội dung mang tính lý thuyết nhưng rất thực tiễn, phù hợp cho kỹ sư đã quen với **LLM** basics và sẵn sàng triển khai hệ thống **agent**.
...
#### Bài phát biểu của Nguyen Tuan Thinh
...
- Slides thực dụng và ví dụ rõ ràng — hữu ích cho cả beginners và advanced users tập trung vào cost và quality.
...
- Trình bày một dự án **AIoT** thực tế: một hệ thống quản lý locker tự động sử dụng sensors, **RFID**, và **AI**.
- Chia sẻ kiến trúc hardware + cloud (**AWS**), data flow, costs, và một working demo.

### Diễn giả

- Aiden Dinh — Katalon Operation Engineer, "AI-Powered Projects – AIoT Project: Locker Management".

#### Bài phát biểu của Aiden Dinh
...
- Một showcase-style talk: problem → mission → architecture → cost → demo — inspiration tuyệt vời cho projects.
